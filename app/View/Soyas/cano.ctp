...
<ul>
<li> <?php echo $this->Html->link("Reporte", array('action'=>'reportecano', $this->data['Soya']['id'])); ?> </li>
</ul>
<ul>
<li><?php 
		...
		<?php foreach ($this->data['SoyaProductorCompra'] as $soyaproductorcompra): ?>
			...
			if (date("m", $year)==$i) { ?>
				<?php if (date("Y", $year)==$var) { ?>
					...
					<?php } ?>
					<?php if ($soyaproductorcompra['producto']=='GRNO DE GIRASOL') { ?>
