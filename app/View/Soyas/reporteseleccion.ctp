...
<?php echo $this->Form->create('Soya', array('action' => 'reportecomprafiltro', $this->request->data['Soya']['id'])); ?>
<table>
	  <td style="float: left; width: 40%;">	  
		...
		?>
		<?php
			$options = array(1 => 'GRANO DE SOYA', 'GRANO DE GIRASOL');
			$selected = array();
...
<?php 
	foreach ($numbers as $i => $inarray){
		if (in_array($inarray, $numbers)) {
