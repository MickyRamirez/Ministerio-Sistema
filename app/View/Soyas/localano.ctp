...
<ul>
<li><?php echo $this->Html->link("Reporte", array('action'=>'reportevlano', $this->data['Soya']['id'])); ?></li>
</ul>
<ul>
<li><?php 
...
echo $this->Form->hidden('id', array('value' => $this->data['Soya']['id']));
 ?>
<h3>Panel de Ventas Locales Anuales <br/> 
...
<table>
		...
		<?php foreach ($this->data['SoyaProductorProduccion'] as $soyaproductorproduccion): ?>
				...
				date("Y", $year);
				if (date("Y", $year)==$var) { ?>
				 <?php if ($soyaproductorproduccion['operacion']=='Venta Local') { ?>
