...
<h3>Panel de Control <br/>
	...
	Todas las Empresas Oleaginosas por Régimen</h3>
...
<?php for ($i=1; $i <= 2 ; $i++) { ?>
		<?php 
		$tar='';
		if ($i==2) { echo ' <h2>Campaña de Invierno</h2>';
			$tar='INVIERNO';
		}
		if ($i==1) { echo ' <h2>Campaña de Verano</h2>'; 
			$tar='VERANO';
		}
		?>
        ...
        <?php if($i==2){
			...
			<?php if (date("Y", $year)==$anopas) { ?>
                <?php if (date("m", $year)=='01') { ?>
					<?php if($soya['SoyaProveedor']['regimen']=='RAU'){ ?>
						<?php $granosoya=$granosoya+$soya['SoyaProductorCompra']['toneladas'];?>
						<?php $regimenrau=$regimenrau+$soya['SoyaProductorCompra']['total'];?>
					<?php } ?>
					<?php if ($soya['SoyaProveedor']['regimen']=='GENERAL') { ?>
...
<ul>
<li><?php echo $this->Html->link( "Reporte",   array('action'=>'reporteregimengeneralsoya') ); ?></li> 
</ul>
<ul>
<li><?php 
