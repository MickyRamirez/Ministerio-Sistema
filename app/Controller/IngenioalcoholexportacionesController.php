<?php
class IngenioalcoholexportacionesController extends AppController {
	...
	public function add()
	{
        ...
        $this->request->data['IngenioAlcoholExportacion']['user_id'] = $this->Auth->user('id');
        if ($this->IngenioAlcoholExportacion->save($this->request->data)) {
	...
	public function index()
	{
		...
		$conditions=$this->paginate = array(
            'limit' => 30,
            'order' => array('IngenioAlcoholExportacion.id' => 'asc' ),
            'conditions' => array('IngenioAlcoholExportacion.user_id' =>  $this->Auth->user('id'))
        );
		$result = $this->IngenioAlcoholExportacion->find('all', $conditions);
		if (empty($result)){
		    $this->redirect(array('action' => 'add'));
		}else{
			$ingenioalcoholexportaciones = $this->paginate('IngenioAlcoholExportacion');
        	$this->set(compact('ingenioalcoholexportaciones'));
		}
	}
    ...
    public function excel() {
        ...
        if ($this->request->is('post')) {
            ...
            $datos->read($this->request->data['IngenioAlcoholExportacion']['excel']['tmp_name']);
