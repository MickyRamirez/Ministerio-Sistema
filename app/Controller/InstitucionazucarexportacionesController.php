<?php
class InstitucionazucarexportacionesController extends AppController {
	...
	public function add()
	{
        ...
        $this->request->data['InstitucionAzucarExportacion']['user_id'] = $this->Auth->user('id');
        $this->request->data['InstitucionAzucarExportacion']['total'] = $this->request->data['InstitucionAzucarExportacion']['refinada']+$this->request->data['InstitucionAzucarExportacion']['cruda'];
        if ($this->InstitucionAzucarExportacion->save($this->request->data)) {
	...
	public function index()
	{
		...
		$conditions=$this->paginate = array(
            'limit' => 30,
            'order' => array('InstitucionAzucarExportacion.id' => 'asc' ),
            'conditions' => array('InstitucionAzucarExportacion.user_id' =>  $this->Auth->user('id'))
        );
		$result = $this->InstitucionAzucarExportacion->find('all', $conditions);
		
		if (empty($result)){
		    $this->redirect(array('action' => 'add'));
		}else{
			$institucionazucarexportaciones = $this->paginate('InstitucionAzucarExportacion');
        	$this->set(compact('institucionazucarexportaciones'));
		}
	}
    ...
    public function excel() {
        ...
        if ($this->request->is('post')) {
            ...
            $datos->read($this->request->data['InstitucionAzucarExportacion']['excel']['tmp_name']);
