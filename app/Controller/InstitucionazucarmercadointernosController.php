<?php
class InstitucionazucarmercadointernosController extends AppController {
	...
	public function add()
	{
        ...
        $this->request->data['InstitucionAzucarMercadoInterno']['user_id'] = $this->Auth->user('id');
        $this->request->data['InstitucionAzucarMercadoInterno']['total'] = $this->request->data['InstitucionAzucarMercadoInterno']['refinada']+$this->request->data['InstitucionAzucarMercadoInterno']['cruda'];
        $this->request->data['InstitucionAzucarMercadoInterno']['razonsocial'] = strtoupper($this->request->data['InstitucionAzucarMercadoInterno']['razonsocial']);
        if ($this->InstitucionAzucarMercadoInterno->save($this->request->data)) {
	...
	public function index()
	{
		...
		$conditions=$this->paginate = array(
            'limit' => 30,
            'order' => array('InstitucionAzucarMercadoInterno.id' => 'asc' ),
            'conditions' => array('InstitucionAzucarMercadoInterno.user_id' =>  $this->Auth->user('id'))
        );
		$result = $this->InstitucionAzucarMercadoInterno->find('all', $conditions);
		
		if (empty($result)){
		    $this->redirect(array('action' => 'add'));
		}else{
			$institucionazucarmercadointernos = $this->paginate('InstitucionAzucarMercadoInterno');
        	$this->set(compact('institucionazucarmercadointernos'));
		}
	}
    ...
    public function excel() {
        ...
        if ($this->request->is('post')) {
            ...
            $datos->read($this->request->data['InstitucionAzucarMercadoInterno']['excel']['tmp_name']);
