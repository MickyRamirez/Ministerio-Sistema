...
class SoyasubrubrosController extends AppController {
	...
	public function add()
	{
        	$this->loadModel('SoyaSubrubro');
                ...
                $this->set('rubros', $this->SoyaRubro->find('list',array(
                                                'fields'=>array('id','rubro')
                )
                ));
        	if ($this->request->is('post')) {
                $this->request->data['SoyaSubrubro']['subrubro'] = strtoupper($this->request->data['SoyaSubrubro']['subrubro']);
                    if ($this->SoyaSubrubro->save($this->request->data)) {
	...
	public function index()
	{
            ...
            'limit' => 30,
            'order' => array('SoyaSubrubro.id' => 'asc' )
        );
            $result = $this->SoyaSubrubro->find('all', $conditions);
		
		if (empty($result)){
		    $this->redirect(array('action' => 'add'));
		}else{
                    $soyasubrubros = $this->paginate('SoyaSubrubro');
                    $this->set(compact('soyasubrubros'));
		}
	}
    public function reporterubros()
    {
            $this->loadModel('SoyaSubrubro');
            $conditions=$this->paginate = array(
            'limit' => 30,
        ...
        $this->render('reporterubros','export_xls');
    }
