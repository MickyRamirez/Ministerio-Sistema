<?php
class InstitucionalcoholexportacionesController extends AppController {
	...
	public function add()
	{
        ...
        $this->request->data['InstitucionAlcoholExportacion']['user_id'] = $this->Auth->user('id');
        if ($this->InstitucionAlcoholExportacion->save($this->request->data)) {
	...
	public function index()
	{
		...
		$conditions=$this->paginate = array(
            'limit' => 30,
            'order' => array('InstitucionAlcoholExportacion.id' => 'asc' ),
            'conditions' => array('InstitucionAlcoholExportacion.user_id' =>  $this->Auth->user('id'))
        );
		$result = $this->InstitucionAlcoholExportacion->find('all', $conditions);
		
		if (empty($result)){
		    $this->redirect(array('action' => 'add'));
		}else{
			$institucionalcoholexportaciones = $this->paginate('InstitucionAlcoholExportacion');
        	$this->set(compact('institucionalcoholexportaciones'));
		}
	}
    ...
    public function excel() {
        ...
        if ($this->request->is('post')) {
            ...
            $datos->read($this->request->data['InstitucionAlcoholExportacion']['excel']['tmp_name']);
