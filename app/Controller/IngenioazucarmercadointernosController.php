<?php
class IngenioazucarmercadointernosController extends AppController {
	...
	public function add()
	{
		...
		if ($this->request->is('post')) {
             $this->request->data['IngenioAzucarMercadoInterno']['total'] = $this->request->data['IngenioAzucarMercadoInterno']['refinada']+$this->request->data['IngenioAzucarMercadoInterno']['cruda'];
             $this->request->data['IngenioAzucarMercadoInterno']['razonsocial'] = strtoupper($this->request->data['IngenioAzucarMercadoInterno']['razonsocial']);

        $this->request->data['IngenioAzucarMercadoInterno']['user_id'] = $this->Auth->user('id');
        if ($this->IngenioAzucarMercadoInterno->save($this->request->data)) {
	...
	public function index()
	{
		...
		$conditions=$this->paginate = array(
            'limit' => 30,
            'order' => array('IngenioAzucarMercadoInterno.id' => 'asc' ),
            'conditions' => array('IngenioAzucarMercadoInterno.user_id' =>  $this->Auth->user('id'))
        );


		$result = $this->IngenioAzucarMercadoInterno->find('all', $conditions);
		
		if (empty($result)){
		    $this->redirect(array('action' => 'add'));
		}else{
			$ingenioazucarmercadointernos = $this->paginate('IngenioAzucarMercadoInterno');
        	$this->set(compact('ingenioazucarmercadointernos'));
		}
	}
    ...
    public function excel() {
        ...
        if ($this->request->is('post')) {
            ...
            $datos->read($this->request->data['IngenioAzucarMercadoInterno']['excel']['tmp_name']);
