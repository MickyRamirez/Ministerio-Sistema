<?php
class InstitucionalcoholmercadointernosController extends AppController {
	...
	public function add()
	{
        ...
        $this->request->data['InstitucionAlcoholMercadoInterno']['user_id'] = $this->Auth->user('id');
        $this->request->data['InstitucionAlcoholMercadoInterno']['razonsocial'] = strtoupper($this->request->data['InstitucionAlcoholMercadoInterno']['razonsocial']);
        if ($this->InstitucionAlcoholMercadoInterno->save($this->request->data)) {
	...
	public function index()
	{
		...
		$conditions=$this->paginate = array(
            'limit' => 30,
            'order' => array('InstitucionAlcoholMercadoInterno.id' => 'asc' ),
            'conditions' => array('InstitucionAlcoholMercadoInterno.user_id' =>  $this->Auth->user('id'))
        );
		$result = $this->InstitucionAlcoholMercadoInterno->find('all', $conditions);
		
		if (empty($result)){
		    $this->redirect(array('action' => 'add'));
		}else{
			$institucionalcoholmercadointernos = $this->paginate('InstitucionAlcoholMercadoInterno');
        	$this->set(compact('institucionalcoholmercadointernos'));
		}
	}
    ...
    public function excel() {
        ...
        if ($this->request->is('post')) {
            ...
            $datos->read($this->request->data['InstitucionAlcoholMercadoInterno']['excel']['tmp_name']);
