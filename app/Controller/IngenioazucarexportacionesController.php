<?php
class IngenioazucarexportacionesController extends AppController {
	...
	public function add()
	{
        ...
        $this->request->data['IngenioAzucarExportacion']['user_id'] = $this->Auth->user('id');
        $this->request->data['IngenioAzucarExportacion']['total'] =$this->request->data['IngenioAzucarExportacion']['refinada']+$this->request->data['IngenioAzucarExportacion']['cruda'];
        if ($this->IngenioAzucarExportacion->save($this->request->data)) {
	...
	public function index()
	{
		...
		$conditions=$this->paginate = array(
            'limit' => 30,
            'order' => array('IngenioAzucarExportacion.id' => 'asc' ),
            'conditions' => array('IngenioAzucarExportacion.user_id' =>  $this->Auth->user('id'))
        );


		$result = $this->IngenioAzucarExportacion->find('all', $conditions);
		
		if (empty($result)){
		    $this->redirect(array('action' => 'add'));
		}else{
			$ingenioazucarexportaciones = $this->paginate('IngenioAzucarExportacion');
        	$this->set(compact('ingenioazucarexportaciones'));
		}
    ...
    public function excel() {
        ...
        if ($this->request->is('post')) {
            ...
            $datos->read($this->request->data['IngenioAzucarExportacion']['excel']['tmp_name']);
