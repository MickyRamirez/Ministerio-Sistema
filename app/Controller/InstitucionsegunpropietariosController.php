<?php
class InstitucionsegunpropietariosController extends AppController {
	...
	public function add()
	{
        ...
        $this->request->data['InstitucionSegunPropietario']['user_id'] = $this->Auth->user('id');
        $this->request->data['InstitucionSegunPropietario']['total'] = $this->request->data['InstitucionSegunPropietario']['refinada']+$this->request->data['InstitucionSegunPropietario']['cruda'];
        $this->request->data['InstitucionSegunPropietario']['nombre'] = strtoupper($this->request->data['InstitucionSegunPropietario']['nombre']);
        if ($this->InstitucionSegunPropietario->save($this->request->data)) {
	...
	public function index()
	{
		...
		$conditions=$this->paginate = array(
            'limit' => 30,
            'order' => array('InstitucionSegunPropietario.id' => 'asc' ),
            'conditions' => array('InstitucionSegunPropietario.user_id' =>  $this->Auth->user('id'))
        );
		$result = $this->InstitucionSegunPropietario->find('all', $conditions);
		
		if (empty($result)){
		    $this->redirect(array('action' => 'add'));
		}else{
			$institucionsegunpropietarios = $this->paginate('InstitucionSegunPropietario');
        	$this->set(compact('institucionsegunpropietarios'));
		}
	}
    ...
    public function excel() {
        ...
        if ($this->request->is('post')) {
            ...
            $datos->read($this->request->data['InstitucionSegunPropietario']['excel']['tmp_name']);
