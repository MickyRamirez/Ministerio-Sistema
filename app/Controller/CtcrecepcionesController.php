...
<?php
class CtcrecepcionesController extends AppController {
	...
	public function add()
	{
        ...
        $this->request->data['CtcRecepcion']['user_id'] = $this->Auth->user('id');
        if ($this->CtcRecepcion->save($this->request->data)) {
	...
	public function index()
	{
		...
		$conditions=$this->paginate = array(
            'limit' => 30,
            'order' => array('CtcRecepcion.id' => 'asc' ),
            'conditions' => array('CtcRecepcion.user_id' =>  $this->Auth->user('id'))
        );
		$result = $this->CtcRecepcion->find('all', $conditions);
		
		if (empty($result)){
		    $this->redirect(array('action' => 'add'));
		}else{
			$ctcrecepciones = $this->paginate('CtcRecepcion');
        	$this->set(compact('ctcrecepciones'));
		}
    ...
    public function excel() {
        ...
        if ($this->request->is('post')) {
            ...
            $datos->read($this->request->data['CtcRecepcion']['excel']['tmp_name']);
