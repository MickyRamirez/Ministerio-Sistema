...
<?php
class IngenioproduccionesController extends AppController {
	...
	public function add()
	{
        ...
        $this->request->data['IngenioProduccion']['user_id'] = $this->Auth->user('id');
        $this->request->data['IngenioProduccion']['totalquintal'] = $this->request->data['IngenioProduccion']['refinada']+$this->request->data['IngenioProduccion']['cruda'];
        $this->request->data['IngenioProduccion']['totallitros'] = $this->request->data['IngenioProduccion']['producciondirecta']+$this->request->data['IngenioProduccion']['subproducto'];
        if ($this->IngenioProduccion->save($this->request->data)) {
	...
	public function index()
	{
		...
		$conditions=$this->paginate = array(
            'limit' => 30,
            'order' => array('IngenioProduccion.id' => 'asc' ),
            'conditions' => array('IngenioProduccion.user_id' =>  $this->Auth->user('id'))
        );


		$result = $this->IngenioProduccion->find('all', $conditions);
		
		if (empty($result)){
		    $this->redirect(array('action' => 'add'));
		}else{
			$ingenioproducciones = $this->paginate('IngenioProduccion');
        	$this->set(compact('ingenioproducciones'));
		}
    ...
    public function excel() {
        ...
        if ($this->request->is('post')) {
            ...
            $datos->read($this->request->data['IngenioProduccion']['excel']['tmp_name']);
